...
void setup() {
  ...
  mqttClient.setId("serial-receiver");
  ...
  mqttClient.beginMessage("receiver");
...
void loop() {
  ...
  //mqttClient.print(String(Serial.available()));
  //mqttClient.endMessage();
  ...
  while (! messageReceived) {
    ...
    while (Serial.available() > 0) {
      ...
      // Has the end of message char been received
      if (char(incomingByte) == 't') {
        messageReceived = true;
        break;
      }
      else {
        mqttClient.print(char(incomingByte));
      }
    }
